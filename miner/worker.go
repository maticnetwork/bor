...
type generateParams struct {
	...
	slotNum                   *uint64                 // The slot number (EIP-7843 SLOTNUM, amsterdam field). Dormant on Bor — never populated, so header.SlotNumber stays nil.
...
func (w *worker) makeHeader(genParams *generateParams, waitOnPrepare bool) (*types.Header, common.Address, error) {
	...
	// EIP-7843 SLOTNUM. Amsterdam is dormant on Bor (AmsterdamBlock nil), so this
	// never fires; when Amsterdam is scheduled, Bor's producer must supply slotNum.
	if w.chainConfig.IsAmsterdam(header.Number) {
		if genParams.slotNum == nil {
			return nil, common.Address{}, errors.New("no slot number set post-amsterdam")
		}
