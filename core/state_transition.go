...
func ApplyMessage(evm *vm.EVM, msg Message, gp *GasPool, interruptCh *chan struct{}) (*ExecutionResult, error) {
	return NewStateTransition(evm, msg, gp).TransitionDb(interruptCh)
}
...
func (st *StateTransition) TransitionDb(interruptCh *chan struct{}) (*ExecutionResult, error) {
	...
	if contractCreation {
		...
		ret, st.gas, vmerr = st.evm.Call(sender, st.to(), st.data, st.gas, st.value, interruptCh)
	}
