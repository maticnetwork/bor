...
func testEthTransferLogs(t *testing.T, value uint64) {
	...
	// Amsterdam is wired dormant on Bor and gated by block number rather than
	// upstream's timestamp, so activate it from genesis for this test.
	config.AmsterdamBlock = new(big.Int)
	...
	// Bor emits its own LogTransfer from the 0x1010 fee address after every
	// core.Transfer, plus a LogFeeTransfer for the gas payment, so the EIP-7708
	// logs are interleaved rather than contiguous. Index records where each one
	// lands in the full receipt, which pins the interleaving.
	indices := []uint{0, 2, 3, 5, 6}
	if value == 0 {
		// no ETH transfer logs expected with zero value, and Bor's own transfer
		// logs are skipped for zero amounts too
	...
	for _, log := range receipts[0][0].Logs {
		if log.Address != feeAddress {
			gotLogs = append(gotLogs, log)
		}
	}
	if len(expLogs) != len(gotLogs) {
		t.Fatalf("Incorrect number of logs (expected: %d, got: %d)", len(expLogs), len(gotLogs))
	}
	for i, log := range gotLogs {
