...
func New(config Config, chain BlockChain, options ...func(pool *LegacyPool)) *LegacyPool {
	...
	log.Info("[manav] sanitized config", "pricelimit", config.PriceLimit)
...
func (pool *LegacyPool) SetGasTip(tip *big.Int) {
	...
	log.Info("[manav] updating gas limit", "old", old.Uint64(), "new", tip.Uint64())
...
func (pool *LegacyPool) validateTxBasics(tx *types.Transaction, local bool) error {
	...
	log.Info("[manav] validating transaction", "minTip", opts.MinTip.Uint64(), "tx minTip", tx.GasTipCap().Uint64())
	if local {
		log.Info("[manav] setting mintip to 0 as it's local tx")
