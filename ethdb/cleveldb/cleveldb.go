...
func New(file string, cache int, handles int, namespace string, readonly bool) (*Database, error) {
	return NewCustom(file, namespace, func(options *levigo.Options) {
		...
		// Set default options
		options.SetMaxOpenFiles(handles)
		options.SetWriteBufferSize(cache / 4 * opt.MiB) // Two of these are used internally

		if readonly {
			log.Info("Readonly mode is not present in cleveldb")
		}
	})
}
...
func NewCustom(file string, namespace string, customize func(*levigo.Options)) (*Database, error) {
	options := configureOptions(customize)
	logger := log.New("database", file)

	db, err := levigo.Open(file, options)
	if err != nil {
		return nil, err
	}
...
func configureOptions(customizeFn func(*levigo.Options)) *levigo.Options {
	options := levigo.NewOptions()
	options.SetCache(levigo.NewLRUCache(1 << 30))
...
func (db *Database) Get(key []byte) ([]byte, error) {
	...
	res, err := db.db.db.Get(db.db.ro, key)
	if err != nil {
		return nil, err
	}
	return res, nil
...
func (db *Database) Delete(key []byte) error {
	...
	if err := db.db.db.Delete(db.db.wo, key); err != nil {
		return err
	}
	return nil
}
...
func (db *Database) NewIterator(prefix []byte, start []byte) ethdb.Iterator {
	return db.db.db.NewIterator(levigo.NewReadOptions())
}
...
func (db *Database) NewSnapshot() (ethdb.Snapshot, error) {
	snap := db.db.db.NewSnapshot()

	return &snapshot{db: db.db.db, ro: db.db.ro, snap: snap}, nil
}

// Stat returns a particular internal stat of the database.
func (db *Database) Stat(property string) (string, error) {
	return db.db.db.PropertyValue(property), nil
}
...
func (db *Database) Compact(start []byte, limit []byte) error {
	db.db.db.CompactRange(levigo.Range{Start: start, Limit: limit})
	return nil
}
...
func (db *Database) meter(refresh time.Duration) {
	...
	for i := 1; errc == nil && merr == nil; i++ {
		...
		writedelay := db.db.db.PropertyValue("leveldb.writedelay")
		...
		ioStats := db.db.db.PropertyValue("leveldb.iostats")
		...
		compCount := db.db.db.PropertyValue("leveldb.compcount")
...
func (snap *snapshot) Has(key []byte) (bool, error) {
	bytes, err := snap.db.Get(snap.ro, key)
	if err != nil {
		return false, err
	}
	return bytes != nil, nil
...
func (snap *snapshot) Get(key []byte) ([]byte, error) {
	return snap.db.Get(snap.ro, key)
}

// // Release releases associated resources. Release should always succeed and can
// // be called multiple times without causing error.
func (snap *snapshot) Release() {
	snap.db.ReleaseSnapshot(snap.snap)
}
