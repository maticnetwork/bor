...
| EIP-7708 (ETH transfers as logs) | #33645 (`b87340a85`) | 19 (v1.17.2 4/4) | `params/protocol_params.go`, `core/types/log.go`, `core/evm.go`, `core/state_transition.go`, `core/state/{statedb,statedb_hooked,parallel_statedb}.go`, `core/vm/{evm,instructions,interface}.go` | `EthTransferLogEvent` / `EthBurnLogEvent` topic constants + `types.EthTransferLog` / `types.EthBurnLog`; `Transfer`/`TransferFunc` gain `*params.Rules`; emission sites in `core.Transfer`, `core.EthereumTransfer`, `opSelfdestruct6780`, and `StateDB.EmitLogsForBurnAccounts` (called from `stateTransition.execute`), each behind `rules.IsAmsterdam` | **enable-by-block** — dormant | yes — `IsAmsterdam` false on every preset → no system log is ever emitted and receipts are byte-identical to pre-merge. **Four enable-time consequences, all Bor-specific.** (1) **Duplicate transfer logs:** Bor already emits its own `LogTransfer` from the 0x1010 fee address for every `core.Transfer`, so with Amsterdam active a plain value transfer produces *both* that and the EIP-7708 system log — a bloom and receipt-size change, and a product decision to take before enabling. (2) **V1/V2 log order — currently inverted, POS-3716.** The emission is placed at the same point relative to the balance change on Bor's serial path, its V2 BlockSTM fast path, and `EthereumTransfer`, but that is not sufficient: `RecordTransfer` captures `LogIdx = len(s.logs)` *before* `emitEthTransferLog` appends, so the 7708 log lands at exactly `s.logs[tr.LogIdx]`, and `tryEmitTransferAt` flushes only `< tr.LogIdx` before emitting the 0x1010 log. Serial order is [7708, 0x1010]; V2 order is [0x1010, 7708] — a receipt-root split between the engines. Note the obvious fix is unsafe: widening the boundary to `<= tr.LogIdx` reorders receipts on live mainnet today, because pre-Amsterdam `s.logs[tr.LogIdx]` is an unrelated log. Verified on #2328 from an AI review finding. (3) **Selfdestruct depends on a declined PR:** upstream's burn/transfer branch reads `StateDB.IsNewContract` from the deferred #32919 rework; Bor instead takes the `wasNewContract` bool that its own `SelfDestruct6780` already returns. If #32919 is ever adopted, that adaptation must be revisited rather than blindly replaced. (4) **EmitLogsForBurnAccounts sits on the wrong side of the fee credit, POS-3716.** `stateTransition.execute` calls it right after the `if !st.noFeeBurnAndTip` block, and the engines set that flag differently — serial (`ApplyMessageNoFeeBurnOrTip`) credits the coinbase and burnt contract externally, afterwards; V2 (`ApplyMessageNoFeeLog`) credits inline, before. A coinbase or burnt-contract address that self-destructed in the same transaction with residual balance would get an `EthBurnLog` under V2 and not under serial. Unreachable in practice today — both addresses are fixed pre-existing values, and post-Rio the coinbase is a governance config value rather than a per-block validator choice. Needs N-1/N/N+1 boundary tests and Erigon parity at enable time, plus V2 BlockSTM coverage with Amsterdam active — `core/eth_transfer_logs_test.go` drives only the serial path, which is why CI is blind to (2) and (4). |
