...
func (api *API) GetCurrentValidators() ([]*set.Validator, error) {
	snap, err := api.GetSnapshot(nil)
	if err != nil {
		return make([]*set.Validator, 0), err
	}
...
func (api *API) GetRootHash(start uint64, end uint64) (string, error) {
	...
	if start > end || end > currentHeaderNumber {
		return "", &set.InvalidStartEndBlockError{Start: start, End: end, CurrentHeader: currentHeaderNumber}
	}
