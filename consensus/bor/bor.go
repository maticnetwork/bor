...
func (c *Bor) CommitStates(
	...
	fmt.Println("PSP - current block", number, "to", to, "hash", chain.Chain.GetHeaderByNumber(number).Hash(), "Time", chain.Chain.GetHeaderByNumber(number).Time, "diff", header.Difficulty.Uint64(), chain.Chain.GetHeaderByNumber(number).Difficulty, "to hash", chain.Chain.GetHeaderByNumber(number-c.config.CalculateSprint(number)).Hash())
