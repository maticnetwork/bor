...
func (c *Bor) CommitStates(
	...
	fmt.Println("PSP - current block", number, "to", to, "hash", chain.Chain.GetHeaderByNumber(number).Hash(), "diff", header.Difficulty.Uint64(), "to hash", chain.Chain.GetHeaderByNumber(number-c.config.CalculateSprint(number)).Hash())
