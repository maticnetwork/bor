...
func (c *ChainSpanner) GetCurrentValidatorsByState(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, state *state.StateDB, blockNumber uint64) ([]*valset.Validator, error) {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
