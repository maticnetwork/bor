...
type ChainConfig struct {
	...
	ForkXXXBlock        *big.Int `json:"forkXXXBlock,omitempty"`        // ForkXXX switch block (nil = no fork, 0 = already on berlin)
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v Petersburg: %v Istanbul: %v, Muir Glacier: %v, Berlin: %v, London: %v, ForkXXX %v, Engine: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
// IsForkXXX returns whether num is either equal to the ForkXXX fork block or greater.
func (c *ChainConfig) IsForkXXX(num *big.Int) bool {
	return isForked(c.ForkXXXBlock, num)
}
...
type Rules struct {
	...
	IsBerlin, IsLondon, IsForkXXX                           bool
}
