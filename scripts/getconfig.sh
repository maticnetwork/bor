...
confPath=${startPath%start.sh}"config.toml"
...
while read line || [ -n "$line" ]; do
    ...
    if [[ $line == bor* ]]; then
        ...
        # later this `-` can be changed to `server`
        flags="-"${line#*-}
    fi
done < $startPath
...
then
    ...
    echo "nither JSON nor TOML StaticNodes found"
fi
...
then
    ...
    echo "nither JSON nor TOML TrustedNodes found"
fi
