...
func (s *SearcherAPI) CallBundle(ctx context.Context, args CallBundleArgs) (map[string]interface{}, error) {
	...
	for i, tx := range txs {
		...
		interruptCtx := vm.SetCurrentTxOnContext(ctx, tx.Hash())
		receipt, result, err := core.ApplyTransaction(s.b.ChainConfig(), s.chain, &coinbase, gp, state, header, tx, &header.GasUsed, vmconfig, interruptCtx)
		if err != nil {
